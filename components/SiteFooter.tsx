    ...
    <footer className="relative z-10 border-t border-[#dfe5f2] bg-[#f7f8fb] text-[#080b18]">
      <div className="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:py-14">
        <div className="grid gap-10 border-b border-[#dfe5f2] pb-10 lg:grid-cols-[1.05fr_1.95fr] lg:gap-14">
          <div>
            <Link href="/" className="inline-flex items-center" aria-label="QSentia home">
              <Image
            ...
            <p className="mt-5 max-w-md text-sm leading-6 text-[#4d5870]">
              Live model telemetry, research workflows, and API surfaces for systematic investment
              diligence, benchmark review, and execution monitoring.
            </p>
          ...
          <div className="grid gap-8 sm:grid-cols-2 xl:grid-cols-4">
            {footerColumns.map((column) => (
              <nav key={column.title} aria-label={column.title}>
                <h2 className="text-sm font-bold text-[#080b18]">{column.title}</h2>
        ...
        <div className="flex flex-col gap-5 pt-7 lg:flex-row lg:items-center lg:justify-between">
          <div className="flex max-w-3xl gap-3 text-sm leading-6 text-[#4d5870]">
            ...
            <p>
              Quantitative trading systems involve risk. Historical or paper-trading information
              does not guarantee future results. Data shown only reflects returned source logs.
            </p>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-x-5 gap-y-2 text-sm font-semibold text-[#4d5870]">
            ...
            {legalLinks.map((link) => (
              <Link key={link.href} href={link.href} className="transition-colors hover:text-[#2b36ff]">
                {link.label}
              </Link>
            ))}
