...
import { mergeAdminModelSetting, readAdminModelSettings } from '@/lib/adminModelSettings';
...
export async function buildAdminOverview(request: Request) {
  ...
  const bestPerforming = [...enrichedModels]
    .sort((a, b) => {
      const aSharpe = Number(a.performance.sharpeRatio ?? Number.NEGATIVE_INFINITY);
      const bSharpe = Number(b.performance.sharpeRatio ?? Number.NEGATIVE_INFINITY);
      if (bSharpe !== aSharpe) return bSharpe - aSharpe;
      return Number(b.performance.annualizedReturn ?? 0) - Number(a.performance.annualizedReturn ?? 0);
    });
  ...
  const metrics = {
    ...
    activePublicModels: enrichedModels.filter(
      (model) => model.settings.accessStatus === 'active' && model.settings.visibility === 'public'
    ).length,
    soldModels: enrichedModels.reduce((sum, model) => sum + model.sales.soldUnits, 0),
    activeTrials: enrichedModels.reduce((sum, model) => sum + model.sales.trialCount, 0),
    modelViews: enrichedModels.reduce((sum, model) => sum + model.sales.views, 0),
    ...
    openTickets: backOffice.tickets.filter((ticket) => ticket.status !== 'resolved').length,
    openRoles: backOffice.careerRoles.filter((role) => role.status === 'open').length,
    candidates: backOffice.applications.length,
    activeOffers: backOffice.offers.filter((offer) => offer.status === 'active').length,
    offerRedemptions: backOffice.offers.reduce((sum, offer) => sum + offer.redemptions, 0),
  };
